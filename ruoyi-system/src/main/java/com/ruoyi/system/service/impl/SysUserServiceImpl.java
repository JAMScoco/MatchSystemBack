...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Override
    @Transactional
    public int insertReviewer(SysUser user) {
        // 新增用户信息
        int rows = userMapper.insertUser(user);
        //插入角色信息
        insertUserRole(user);
    ...
    @Override
    public int updateReviewer(SysUser user) {
        return userMapper.updateReviewer(user);
    }

    @Override
    public Map<String, Object> genAssignData(List<SysUser> reviewers, Long reviewNumber, List<String> waitReviewWorkIds) {
        Map<String, Object> result = new HashMap<>();
        //生成表格头部
        List<String> previewAssignHeader = new ArrayList<>();
        for (int i = 1; i <= reviewNumber; i++) {
            previewAssignHeader.add("分数" + i);
        }
        result.put("previewAssignHeader", previewAssignHeader);
        List<Map<String, Object>> previewAssignData = new ArrayList<>();

        //生成表格数据
        int cursor = 0;

        for (String workId : waitReviewWorkIds) {
            Map<String, Object> item = new HashMap<>();
            item.put("workId", workId);
            for (int i = 1; i <= reviewNumber; i++) {
                if (cursor >= reviewers.size()) {
                    cursor = 0;
                }
                item.put("score_" + i, reviewers.get(cursor++));
            }
